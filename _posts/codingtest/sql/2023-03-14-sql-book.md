...
Book 테이블에서 `CATEGORY = 인문`, `PUBLISHED_DATE = 2021`의 값을   
오름차순으로 출력해주는 문제이다.   

여기서 주어지는 타입의 DATE가 시간을 포함해주기 때문에  
DATE_FORMAT을 사용하여 출력의 날짜 형식과 맞춰줘야한다.   

WHERE절을 이용해 인문이라는 카테고리와   
BETWEEN을 이용해 2021년도에 있는 데이터만 추출해온다음   
오름차순으로 정렬해주면 문제 풀이 완료!
