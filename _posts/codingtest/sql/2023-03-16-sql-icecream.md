---
layout: post
title: "[Programmers] 과일로 만든 아이스크림 고르기 (MySQL)"
subtitle: Level 1
categories: SQL
...
`FIRST_HALF`, `INGREDIENT_TYPE`라는 두개의 테이블이 있다.  
두개의 테이블을 JOIN한 다음에 `FIRST_HALF` 테이블에서 주문수량이 3000보다 큰값을 찾고    
`INGREDIENT_TYPE` 테이블에서는 과일을 나타내는 fruit_based를 찾아 내림차순으로 정렬하면된다.
