---
layout: post
title: "[Programmers] 우유와 요거트가 담긴 장바구니 (MySQL)"
subtitle: Level 4
categories: SQL
...
1. WHERE 절을 이용해 우유(Milk)를 필터링한다.
2. 서브쿼리를 이용해 WHERE절 AND조건으로 요거트(Yogurt)를 필터링한다.
3. 우유와 요거트를 중복으로 가지고 있어 중복제거를 위해 DISTINCT를 이용한다.
4. CART_ID를 오름차순으로 정렬한다.
