---
layout: post
title: "[Programmers] 대여 횟수가 많은 자동차들의 월별 대여 횟수 구하기 (MySQL)"
subtitle: Level 3
categories: SQL
...
1. 날짜의 형식이 월만 표시되기 떄문에 MONTH() 함수를 이용한다.
2. RECORDS 열은 2022-08 ~ 2022-10 사이에 총 5회 이상 대여된 수량을 표시하는 행이다. 해당 열을 수량을 표시하기 COUNT를 사용하였다.
3. 2번에 헤당하는 수량을 세기 기준을 정하기 위해서는 GROUP BY를 이용해 그룹핑시켜줘야한다. MONTH(날짜)와 CAR_ID를 기준으로 GROUP BY를 진행하였다. (즉, 같은 날짜와 같은 ID를 가진행의 수량을 RECORDS 열에 표시해준다.)
4. WHERE절을 이용해 2번에 해당하는 날짜와 총5회 이상 대여된 항목을 필터링하여야한다. CAR_ID를 기준으로 SUB쿼리를 이용해 값을 도출해야한다.  
